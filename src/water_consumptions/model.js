...
class WaterConsumption {
  constructor(data) {
    (this.userId = data.userId), (this.date = data.date), (this.quantity = data.quantity);
  }

  async save() {
    await knex('water_consumptions').insert({ user_id: this.userId, quantity: this.quantity });
    return knex.from('water_consumptions').select('*').where('user_id', this.userId);
  }
  ...
  static async getWeeklyConsumption(userId, startDate, endDate) {
    return await knex('water_consumptions')
      ...
      .select(knex.raw('DATE(date) as day'), knex.raw('SUM(quantity) as total_quantity'))
      .groupByRaw('DATE(date)');
  }

  static async getMonthlyConsumption(userId, specificMonth, specificYear) {
    return await knex('water_consumptions')
      .select(knex.raw('EXTRACT(DAY FROM date) as day'), knex.raw('SUM(quantity) as total_quantity'))
      .where('user_id', userId)
